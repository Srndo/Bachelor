...
//                        showVersions(protoID: protoID)
        ...
        .onAppear{
            // in app delegate on start application was diff fetch,
            // if were new record fetch insert it into db
            CloudHelper.shared.insertFetchChangeIntoCoreData(moc: moc, allPhotos: allPhotos, allDAs: DAs)
            print("Photos.count:", allPhotos.count)
            if protoID == -1 {
